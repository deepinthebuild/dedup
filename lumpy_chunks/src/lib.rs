...
pub trait LumpyChunks {
    ...
    ) -> LumpyChunksIter<Self::Contents, P>
    where
        for<'r> P: Fn(&'r [Self::Contents]) -> Option<usize>;
}

pub struct LumpyChunksIter<'a, T: 'a, P>
where
    for<'r> P: Fn(&'r [T]) -> Option<usize>,
{
    slice: &'a [T],
    rough_size: usize,
    break_point_finder: P,
...
impl<'a, T: 'a, P> Iterator for LumpyChunksIter<'a, T, P>
where
    P: Fn(&[T]) -> Option<usize>,
{
    type Item = &'a [T];
